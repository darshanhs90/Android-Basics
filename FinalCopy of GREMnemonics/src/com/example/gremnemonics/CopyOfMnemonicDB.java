...
public class CopyOfMnemonicDB  implements Parcelable{
	...
	public CopyOfMnemonicDB(Context context) {
		ourcontext=context;
	}
	...
	public CopyOfMnemonicDB open() throws SQLException{
		if(ourdbHelper==null){
	...
	public void emptyDB(){
		if(!strin.contentEquals("IMEI"))
			this.strin=this.getIMEI();
	...
	public void resetUID(String stat) throws SQLException{
		ContentValues values=new ContentValues();
		values.put(KEY_STAT, stat);		
		ourdatabase.update(DATABASE_TABLE, values, KEY_ROWID+"="+"4359", null);
	}

	public void resetSurvey(String stat) throws SQLException{
		ContentValues values=new ContentValues();
		values.put(KEY_STAT, stat);		
		ourdatabase.update(DATABASE_TABLE, values, KEY_ROWID+"="+"4360", null);
	}
	...
	public int[] getCount() throws SQLException{
		...
		int a[]=new int[17];
		Cursor cursor = null;
		int count1=0,count101=0,count201=0,count301=0,count401=0,count501=0,count601=0,count701=0,count801=0,count901=0,count1001=0;
		int blueCount=0,redCount=0,orangeRedCount=0,yellowCount=0,lightGreenCount=0,greenCount=0;
		for(int j=1;j<=4362;j++){
			...
			if(cursor!=null){
				...
				if(!cursor.getString(1).contentEquals("BLUE"))
					if(j>=1 && j<=400){
						...
						count201++;
					}else if(j>=1201 && j<=1600){
						count301++;
					}else if(j>=1601 && j<=2000){
						count401++;
					}else if(j>=2001 && j<=2400){
						count501++;
					}else if(j>=2401 && j<=2800){
						count601++;
					}else if(j>=2801 && j<=3200){
						count701++;
					}else if(j>=3201 && j<=3600){
						count801++;
					}else if(j>=3601 && j<=4000){
						count901++;
					}else if(j>=4001 && j<=4362){
						count1001++;
					}
			}
			cursor.close();
		...
		a[11]=count501;a[12]=count601;a[13]=count701;a[14]=count801;a[15]=count901;a[16]=count1001;
		return a;
	}	
	...
	public ArrayList<String> getColors(int i) {
		String[] columns=new String[]{KEY_ROWID,KEY_COLOR};
		Cursor  cursor = null;		
		ArrayList<String> str=new ArrayList<String>();
		if(i!=4362){
	...
	public String getUID() {
		String[] columns=new String[]{KEY_ROWID,KEY_STAT};
		Cursor cursor=ourdatabase.query(DATABASE_TABLE, columns, KEY_ROWID+"="+"4359", null, null, null, null);
		if(cursor!=null){
			cursor.moveToFirst();
			String str=cursor.getString(1);
			cursor.close();
	...
	public String getIMEI(){
		String[] columns=new String[]{KEY_ROWID,KEY_STAT};
		Cursor cursor=ourdatabase.query(DATABASE_TABLE, columns, KEY_ROWID+"="+"4358", null, null, null, null);
		if(cursor!=null){
			cursor.moveToFirst();
			String strnew=cursor.getString(1);
			cursor.close();
			return strnew;
		}
		return null;	
	}
	public void IMEIsetter(){
		ContentValues values=new ContentValues();
		values.put(KEY_STAT, this.strin);		
		ourdatabase.update(DATABASE_TABLE, values, KEY_ROWID+"="+"4358", null);
	}
	public Boolean setIMEI(String str) {
		if(bool==false){
			...
			if(cursor!=null){
				...
				cursor.close();
				if(strnew.contentEquals("IMEI")){
					ContentValues values=new ContentValues();
					values.put(KEY_STAT, str);		
					ourdatabase.update(DATABASE_TABLE, values, KEY_ROWID+"="+"4358", null);
				}
				else{
					if(str.contentEquals(strnew)){
						return true;
					}
					else{
						return false;
	...
	public String getDBStat() throws CursorIndexOutOfBoundsException{
		String[] columns=new String[]{KEY_ROWID,KEY_STAT};
		try{
			Cursor cursor=ourdatabase.query(DATABASE_TABLE, columns, KEY_ROWID+"="+"4362", null, null, null, null);
			if(cursor!=null){
				cursor.moveToFirst();
				String str=cursor.getString(1);
				cursor.close();
	...
	public String getSurveyStat() throws CursorIndexOutOfBoundsException{
		String[] columns=new String[]{KEY_ROWID,KEY_STAT};
		try{
			Cursor cursor=ourdatabase.query(DATABASE_TABLE, columns, KEY_ROWID+"="+"4360", null, null, null, null);
			if(cursor!=null){
				cursor.moveToFirst();
				String str=cursor.getString(1);
				cursor.close();
	...
	public static final Parcelable.Creator<CopyOfMnemonicDB> CREATOR = new Parcelable.Creator<CopyOfMnemonicDB>() {
		public CopyOfMnemonicDB createFromParcel(Parcel in) {
			return new CopyOfMnemonicDB(in);
		}

		public CopyOfMnemonicDB[] newArray(int size) {
			return new CopyOfMnemonicDB[size];
		}
	};

	// example constructor that takes a Parcel and gives you an object populated with it's values
	private CopyOfMnemonicDB(Parcel in) {
		mData = in.readInt();
	}
